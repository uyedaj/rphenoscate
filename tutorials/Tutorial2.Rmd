...
author: "Diego S. Porto, Sergei Tarasov and SCATE team"
date: 'XX June, 2022'
output: pdf_document
---
...
In this second tutorial, we will be using again *rphenoscape* and *rphenoscate* to perform semantic-aware evolutionary analyses of morphological data. But this time, we are going to use an user-provided data set and an external ontology.

In this case, we are going to use a Hymenoptera (e.g., wasps, ants, bees) data set including information not only from absences/presences of anatomical entities, but also their qualities (e.g., shape, structure, composition, etc.). Once again, we will set up appropriate models accounting for trait dependencies based on prior knowledge available from an anatomy ontology, in this case the Hymenoptera Anatomy Ontology (HAO) from Yoder et al. (2010): 10.1371/journal.pone.0015991.
...
For this tutorial, we are going to use the data set from XXXXX (cite), which comprises of XX binary and multistate characters for XX YYY taxa (WWW: WWW).
...
And stochastic character mapping will be performed using the dated phylogeny available from XXX (WWW).
```{r eval = TRUE, include = TRUE, message = FALSE}
# Import tree file.
tree <- read.tree("./data/tree.tre")
```

In the current form, this data set is simply a standard phylogenetic character matrix in NEXUS format. However, with functions from *ontoFAST* it is possible to parse through character statements and extract candidate matches to terms from an user-provided anatomy ontology. For that, we need first to import the HAO .OBO file in R using functions from *ontologyIndex*. 
```{r eval = TRUE, include = TRUE, message = FALSE}
...
And finally, let's process the ontoFAST list of candidate terms to obtain the final term annotations. Note that the filter function will still require the input of an expert to select the preferred terms from the filtered set. 
```{r eval = TRUE, include = TRUE, message = FALSE}
...
Finally, other types of dependencies among different anatomical entities (i.e., parthood-relations) will be dealt only in latter steps.
...
# STEP 3. Check data set for dependencies across different anatomical entities.

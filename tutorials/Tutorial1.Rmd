...
More specifically, we are going to assemble a data set of absences and presences of bones for Characidae fishes (e.g., catfishes), set up appropriate models accounting for trait dependencies based on prior knowledge available in an anatomy ontology (e.g., UBERON), and then simulate character histories through stochastic mapping.
...
Now, let's load the package *rphenoscate*.
...
Most anatomical entities do not contain polymorphic character information, however for some of them, there is a fair amount! For example: 'uroneural 1' (44%) and 'infraorbital 4' (11%). In order to gain some additional information, let's recode all polymorphisms as 'absences' (state:0) since the respective anatomical entities were annotated as 'absent' in at least some individuals of the fish species. Also, let's recode all 'NAs' as missing information '?'.
```{r eval = TRUE, include = TRUE, message = FALSE}
dat[dat == "0 and 1" | dat == "1 and 0"] <- "0"
dat[is.na(dat)] <- "?"
```
